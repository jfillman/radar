...
**Cilium demo cluster** (`scripts/cilium-demo.sh` + `make cilium-demo`): Cilium + Hubble Relay on `kind` with a curl→nginx workload pair, for exercising every connection lane of the Hubble traffic source (`internal/traffic/hubble.go`). Radar dials the relay Service directly when reachable — no `pods/portforward` RBAC — and falls back to a managed port-forward otherwise; the demo makes each lane reachable on demand: default `up` is the common plaintext relay, `tls` switches the relay to TLS whose cert SAN (`*.hubble-relay.cilium.io`) forces the SAN-discovery retry (the AKS shape), `netpol` blocks the direct path to exercise the fallback and the both-remediations error, and `install-radar` builds the current tree into the cluster under the chart's default RBAC to prove `pods/portforward` stays unneeded. **Read `scripts/cilium-demo/README.md` before testing connection changes** — port-forward bypasses NetworkPolicy by design, and Radar caches a failed reachability probe per address, both of which look like bugs if unknown.
