...
**Velero demo cluster** (`scripts/velero-demo.sh` + `make velero-demo`): all 13 Backup phases, the supersession series (a failed nightly superseded by a later success, plus an unlabelled ad-hoc failure), a paused-and-invalid Schedule, an unavailable BackupStorageLocation, a restic BackupRepository, and the rancher `restores.resources.cattle.io` plural collision. **The Velero controller is deliberately scaled to 0**: the interesting phases (`Failed`, `PartiallyFailed`, `FailedValidation`, `WaitingForPluginOperations`) cannot be produced on `kind` without real object storage — a live controller with no bucket yields exactly one outcome, everything failed on credentials — so the fixtures carry their own status and nothing reconciles them away. That also means anything needing a running controller (data-mover CRs, real progress counters, backup logs) is out of reach here. See `scripts/velero-demo/README.md`.
