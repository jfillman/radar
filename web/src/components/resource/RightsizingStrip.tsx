...
function useRequestFit({ kind, namespace, name }: RequestFitProps) {
  useAutoPromConnect()
...
export function RequestFitPanel(props: RequestFitProps) {
  const state = useRequestFit(props)
...
export function RightsizingStrip(props: RequestFitProps) {
  const state = useRequestFit(props)
...
function RequestFitBody({ state, compact }: {
  state: ReturnType<typeof useRequestFit>
  compact: boolean
}) {
        ...
        {Array.from(byContainer.entries()).map(([container, rows]) => (
          <div key={container}>
            ...
            <div className="space-y-1 pl-2">
              {rows.map((row) => <CompactFitRow key={row.resource} row={row} />)}
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="space-y-4">
      {(state.data.scaledToZero || state.data.ownerCoverage === 'current_pods') && (
        <div className="flex flex-wrap gap-2">
          {state.data.scaledToZero && <Badge severity="neutral" size="sm">Scaled to zero · showing retained evidence</Badge>}
          {state.data.ownerCoverage === 'current_pods' && <Badge severity="info" size="sm">Current pods only · confidence capped</Badge>}
        </div>
      )}
      {Array.from(byContainer.entries()).map(([container, rows]) => (
        <div key={container}>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {rows.map((row) => <FitCard key={row.resource} row={row} window={state.data!.window} />)}
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="rounded-md border border-theme-border bg-theme-base/60 p-3">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <span className="text-xs font-semibold uppercase tracking-wide text-theme-text-tertiary">{row.resource}</span>
      ...
      <div className="flex items-baseline gap-2 tabular-nums">
        <div>
          <div className="text-[10px] uppercase tracking-wide text-theme-text-quaternary">Current request</div>
          <div className="text-lg font-semibold text-theme-text-primary">{row.currentRequest ?? 'Unset'}</div>
        </div>
...
function CompactFitRow({ row }: { row: RightsizingRow }) {
  return (
    <div className="flex flex-wrap items-center gap-2 text-xs tabular-nums text-theme-text-tertiary">
      <span className="w-12 text-[10px] uppercase tracking-wide text-theme-text-quaternary">{row.resource}</span>
      <span className="min-w-14 text-theme-text-secondary">{row.currentRequest ?? 'unset'}</span>
      {row.recommendedRequest && <><ArrowRight className="h-3 w-3 text-theme-text-quaternary" /><span className="min-w-14 text-theme-text-primary">{row.recommendedRequest}</span></>}
      {row.observed && <span className="text-[10px] text-theme-text-quaternary">({row.observed.name} {row.observed.formatted})</span>}
      <span className="ml-auto"><FitBadge fit={row.fit} queryError={row.queryError} /></span>
    </div>
  )
}

function RequestFitContext({ data, compact = false }: { data: PrometheusRightsizing; compact?: boolean }) {
  const evidence = data.ownerCoverage === 'ksm_history' ? 'retained ownership' : 'current pods only'
  return <span className="text-[11px] text-theme-text-tertiary">{compact ? data.window : `Last ${data.window} · ${evidence}`}</span>
}

function FitBadge({ fit, queryError }: { fit: RequestFit; queryError?: string }) {
  const presentation = getRequestFitPresentation(fit, queryError)
  return <Badge severity={presentation.severity} size="sm">{presentation.label}</Badge>
}

export function getRequestFitPresentation(fit: RequestFit, queryError?: string) {
  if (queryError) return { label: 'Query failed', severity: 'error' as const }
  const labels: Record<RequestFit, string> = {
    balanced: 'Balanced',
    oversized: 'Oversized',
  ...
  const severities: Record<RequestFit, 'success' | 'info' | 'warning' | 'neutral'> = {
    balanced: 'success',
    oversized: 'info',
...
function FitExplanation({ row }: { row: RightsizingRow }) {
  const message = getRequestFitExplanation(row)
  if (!message) return null
  return <p className="mt-3 text-xs text-theme-text-tertiary">{message}</p>
}

export function getRequestFitExplanation(row: RightsizingRow): string | undefined {
...
function Metric({ label, value }: { label: string; value: string }) {
  return <div><div className="text-theme-text-quaternary">{label}</div><div className="mt-0.5 text-theme-text-secondary">{value}</div></div>
}

function EmptyState({ text, loading = false }: { text: string; loading?: boolean }) {
  return <div className="flex min-h-16 items-center justify-center text-sm text-theme-text-tertiary">{loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{text}</div>
}
